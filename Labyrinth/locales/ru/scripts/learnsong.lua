...
local _SONGTAB =
{
    [SONG_ENERGYFORM]   = "энергетической формы", 
    [SONG_SPIRITFORM]   = "формы духа", -- unused
    [SONG_NATUREFORM]   = "природной формы",
    [SONG_BEASTFORM]    = "звериной формы",
    [SONG_DUALFORM]     = "дуальной формы", -- unused
    [SONG_FISHFORM]     = "формы рыбы",
    [SONG_SUNFORM]      = "солнечной формы",
    [SONG_BIND]         = "связывания",
    [SONG_SHIELD]       = "защиты",
}
...
function update(me, dt)
    if not v.done and v.song then
        ...
        elseif entity_isEntityInRange(me, v.n, 60) then
            ...
            learnSong(v.song)
            setControlHint(string.format("Вы выучили песнь %s!", getSongName(v.song)), 0, 0, 0, 10)
